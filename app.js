...
let shirts = {name: "shirt", amount: 3, previousPrice: 1051, currentPrice: 522, image: "img/t-shirt.png", deliveryNumberForOnce: 2};
let covers = {name: "cover", amount: 210, previousPrice: 11500.235, currentPrice: 10500.235, image: "img/silicone.png", deliveryNumberForOnce: 184};
let pencils = {name: "pencil", amount: 4, previousPrice: 475, currentPrice: 247, image: "img/faber-castell.png", deliveryNumberForOnce: 3};

let possibleDeliveryDates = [["5-6-feb", "5—6 февраля"], ["7-8-feb", "7—8 февраля"]];
        ...
        arrowButtons.forEach((arrowButton) => {
            arrowButton.addEventListener("click", (e) => {
                ...
                if(goodsSection.classList.contains("present")){
                    ...
                    goodsSection.closest(".goods-order-container").querySelector(".absent").classList.toggle("margin-top-16px");
                    if(goodsSection.querySelector("label").querySelector("p").innerHTML === "Выбрать все"){
                        let cartInfo = getItemsInCartInfo();
                        goodsSection.querySelector("label").querySelector("p").innerHTML = cartInfo.goodsNumber + " товаров · "  + cartInfo.finalSumm + " сом";
...
function getItemsInCartInfo(){
    ...
    if(currentPrices&&itemsNumberInputs){
        let finalSumm = 0;
        let goodsNumber = 0;
            ...
            counter.addEventListener("click", (e) => {
                ...
                if(target.closest(".counter-button")){
                    ...
                    for(let i=0; i<itemsArray.length; i++){
                        if(itemName === itemsArray[i].name){
                            itemAmount = itemsArray[i].amount;
                            previousPrice = itemsArray[i].previousPrice;
                            currentPrice = itemsArray[i].currentPrice;
                            chosenItem = itemsArray[i];
                        }
                    }
                    if(target.classList.contains("counter-button-plus")){
                        ...
                        if(itemAmount - value > 2){
                            ...
                            target.closest(".manage").querySelector("p").innerHTML = `Осталось ${itemAmount - value} шт.`;
                            target.closest(".counter").querySelector(".counter-button-minus").classList.remove("disabled");
                    ...
                    target.closest(".item-row").querySelector(".mobile-previous-price-text").textContent = summWithoutSale + " сом";
                    target.closest(".manage-price").querySelector(".price-number").textContent = finalSumm;
                    target.closest(".manage-price").querySelector(".previous-price-text").textContent = summWithoutSale + " сом";
        ...
        if(difference > 0){
            for(let i = 0; i <= deliveryDates.length - 1; i++){
                ...
                if(allDateRowDataItems.includes(chosenItem.name)){
                    ...
                    console.log("создаем элемент товара в строке");
...
function showFinalSumm(){
    ...
    if(finalSummMeaning&&currentPrices&&previousPrices&&itemsNumberInputs&&finalItemsNumber&&finalPreviousPrice&&finalSale){
        let finalSumm = 0;
            ...
            recieverInput.addEventListener("blur", (e) => {
                ...
                if(inputText.length === 0){
                    ...
                    if(recieverInput.id === "first-name"){
                        ...
                        const regex = /^(\+\d{1,2}[-\s]?(\(\d{3,4}\)|\d{3,4})[-\s]?(\d{1,2}[-\s]?){3}\d{1,4}|\+\d{1,2}(\(\d{3,4}\)|\d{3,4})([-\s]\d{1,2}){2}[-\s]?\d{0,3}\d{1,4})$/i;
                        if(!regex.test(inputText)){
                            recieverInput.closest(".group").querySelector("input").classList.add("input-error");
                            recieverInput.closest(".group").querySelector(".error-message").classList.remove("hide");
                        }
...
function formatPhoneNumber(inputString) {
    ...
    if (/^\d{4,}$/.test(cleanNumber)) {
        const formattedNumber = cleanNumber.replace(/(\d{1})(\d{3})(\d{3})(\d{2})(\d{2})/, (_, group1, group2, group3, group4, group5) => {
...
function summCheckboxClicked(){
    if(finalSummCheckbox&&finalSummButton&&finalSummMeaning){
...
function finalSummButtonClicked(){
    if(finalSummButton&&recieverInputs){
...
function changePayingButtonClick(){
    if(payingPopUp&&changePayingButtons){
...
function closePayingPopUp(){
    if(payingPopUp&&payingPopUpClose){
...
function changeDeliveryButtonClick(){
    if(deliveryPopUp&&changeDeliveryButtons){
...
function closeDeliveryPopUp(){
    if(deliveryPopUp&&deliveryPopUpClose){
...
function showChosenAdressCategory(){
    if(tabs&&storeAdresses&&courierAdresses){
        tabs.forEach((tab) => {
...
function chooseAllClicked(){
    if(chooseAllCheckbox&&itemCheckboxes){
    ...
    if(itemCheckboxes){
        itemCheckboxes.forEach((itemCheckbox) => {
            itemCheckbox.addEventListener("change", (e) => {
                showFinalSumm();
                if(itemCheckbox.checked === false){
                    const itemName = itemCheckbox.closest(".item-row").querySelector(".item-name").getAttribute("data-item");
                ...
                for(let deliveryGood of deliveryGoods){
                    ...
                    for(let deliveryItem of deliveryItems){
                        if(deliveryItem.getAttribute("data-item") === itemType){
                            ...
                            let itemsInRow = deliveryItem.closest(".delivery-goods").querySelectorAll(".delivery-items-number").length;
...
function changePriceInPopUp(counterClicked){
    ...
    pricePopUps.forEach(pricePopUp => {
        ...
        if(pricePopUp.closest(".price")){
            previousPrice = parseInt(pricePopUp.closest(".price").querySelector(".previous-price-text").textContent.replaceAll(" ", ""));
            currentPrice = parseInt(pricePopUp.closest(".price").querySelector(".price-number").textContent.replaceAll(" ", ""));
        }else if(pricePopUp.closest(".mobile-price")){
            previousPrice = parseInt(pricePopUp.closest(".mobile-price").querySelector(".mobile-previous-price-text").textContent.replaceAll(" ", ""));
            currentPrice = parseInt(pricePopUp.closest(".mobile-price").querySelector(".mobile-price-number").textContent.replaceAll(" ", ""));
        }
        ...
        pricePopUp.querySelector(".price-pop-up-first-div").querySelector(".price-pop-up-first-p").textContent = `Скидка ${salePerCent}%`;
        pricePopUp.querySelector(".price-pop-up-second-div").querySelector(".price-pop-up-first-p").textContent = `${saleNumber}  сом`;
    });
}
...
function payingCardChanged(){
    ...
    if(popUpPayingButton&&payingRows){
...
function deliveryPlaceChanged(){
    ...
    if(popUpDeliveryButton&&deliveryName&&deliveryInfoDesktopOnly){
        popUpDeliveryButton.addEventListener("click", (e) => {
            let tabs = popUpDeliveryButton.closest(".pop-up-delivery").querySelector(".tabs").querySelectorAll("button");
                ...
                if(adress.querySelector("input").checked === true){
                    for(let tab of tabs){
                        ...
                        if(tab.classList.contains("tabs-chosen")&&buttonType==="store"){
                            deliveryName.textContent = adress.querySelector("p.pop-up-adress-info-text").textContent;
                            deliveryInfoDesktopOnly.textContent = adress.querySelector("p.pop-up-adress-info-text").textContent;
